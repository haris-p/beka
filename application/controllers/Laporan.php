...
class Laporan extends CI_Controller {
	...
	public function index()
	{
		...
		$this->breadcrumb->append_crumb($this->parents, $this->mza_secureurl->setSecureUrl_encode('laporan','index'));

		$data['laporan'] = $this->M_main->get_where('pelanggaran_siswa','status_pelanggaran_siswa',1)->result();

		$data['menu1']='laporan';
		$data['menu2']='laporan';
		$data['content'] = 'laporan/list-laporan.php';
	...
	public function detail_siswa($id){
		...
		$this->breadcrumb->append_crumb(get_apl('nama_sistem'), $this->mza_secureurl->setSecureUrl_encode('Dashboard','index'));
		$this->breadcrumb->append_crumb($this->parents, site_url($this->mza_secureurl->setSecureUrl_encode('laporan','index')));
	...
	public function detailpelanggaran(){
		...
		$this->load->view('admin/laporan/list-laporan.php',$data);
	...
	public function delete(){
		if($this->input->post('id_')){
			...
			if ($this->form_validation->run() == FALSE) {
				...
				redirect(site_url($this->mza_secureurl->setSecureUrl_encode('laporan','index')),'refresh');
			} else {
				$row=$this->M_main->get_where('pelanggaran_siswa','id_pelanggaran_siswa',$this->input->post('id_'))->row_array();
				
				$this->db->where('id_pelanggaran_siswa', $this->input->post('id_'));
				$this->db->delete('pelanggaran_siswa');
				
				history('',$this->parents,'Menghapus Data','','',$this->input->post('id_'));
				
				$this->session->set_flashdata('success','Berhasil menghapus data : '.$row['nis_pelanggaran_siswa']);
				redirect(site_url($this->mza_secureurl->setSecureUrl_encode('laporan','index')),'refresh');
			}
		}
		$data['row']=$this->M_main->get_where('pelanggaran_siswa','id_pelanggaran_siswa',$this->input->post('id'))->row_array();
		$this->load->view('admin/laporan/delete-laporan.php',$data);
	}
	//------------------------------------------------------------
