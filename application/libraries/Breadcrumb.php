...
class Breadcrumb {
	...
	private $_tag_open 		= '<ul class="breadcrumb" style="background-color: #4a8bc2;">';
	...
	function output()
	{
		...
		if ($this->breadcrumbs) {
			...
			foreach ($this->breadcrumbs as $key => $crumb) {
				...
				if (end($an) == $key) {
					$output .= $this->_li_open.'<span style="color:#FFFFFF">'. ($crumb['title']).'</span>' . $this->_li_close;
					
				// else add link and divider
				} else {
					$output .= $this->_li_open.'<a href="' . $crumb['href'] . '" class="active" style="color:#FFFFFF"><b>' . ($crumb['title']) . '</b></a>'.$this->_li_close;
				}
			}
